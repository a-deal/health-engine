...
# Compute test dates relative to today so the 7-day window always works
_today = datetime.utcnow()
_TODAY = _today.strftime("%Y-%m-%d")
_1D_AGO = (_today - __import__("datetime").timedelta(days=1)).strftime("%Y-%m-%d")
_2D_AGO = (_today - __import__("datetime").timedelta(days=2)).strftime("%Y-%m-%d")
_3D_AGO = (_today - __import__("datetime").timedelta(days=3)).strftime("%Y-%m-%d")
_4D_AGO = (_today - __import__("datetime").timedelta(days=4)).strftime("%Y-%m-%d")
_5D_AGO = (_today - __import__("datetime").timedelta(days=5)).strftime("%Y-%m-%d")
_6D_AGO = (_today - __import__("datetime").timedelta(days=6)).strftime("%Y-%m-%d")
_30D_AGO = (_today - __import__("datetime").timedelta(days=30)).strftime("%Y-%m-%d")
_35D_AGO = (_today - __import__("datetime").timedelta(days=35)).strftime("%Y-%m-%d")
...
class TestDetectSourceChanges:
    """Tests use dates relative to today so the 7-day window is stable."""
    ...
    def test_single_source_no_change(self, db_with_andrew):
        ...
        for i in range(7):
            ...
            _insert_wearable(db_with_andrew, "andrew-001", d, "garmin", vo2_max=47.0)
        result = detect_source_changes(db_with_andrew, "andrew-001")
        assert result == {}

    def test_vo2_source_change_detected(self, db_with_andrew):
        """Garmin VO2 for several days, then Apple Health VO2. Should flag vo2_max."""
    ...
    def test_rhr_source_change_detected(self, db_with_andrew):
        ...
        _insert_wearable(db_with_andrew, "andrew-001", _5D_AGO, "oura", rhr=52)
        _insert_wearable(db_with_andrew, "andrew-001", _1D_AGO, "garmin", rhr=48)
    ...
    def test_ignores_changes_outside_window(self, db_with_andrew):
        ...
        _insert_wearable(db_with_andrew, "andrew-001", _35D_AGO, "garmin", vo2_max=47.0)
        _insert_wearable(db_with_andrew, "andrew-001", _30D_AGO, "apple_health", vo2_max=32.3)
        # Recent data all from one source
        _insert_wearable(db_with_andrew, "andrew-001", _1D_AGO, "apple_health", vo2_max=33.0)
    ...
    def test_multiple_metrics_can_change(self, db_with_andrew):
        ...
        _insert_wearable(db_with_andrew, "andrew-001", _5D_AGO, "garmin", vo2_max=47.0, hrv=65)
        _insert_wearable(db_with_andrew, "andrew-001", _1D_AGO, "apple_health", vo2_max=32.3, hrv=45)
...
class TestValidateCoachingClaims:
    ...
    def test_flags_hrv_mention_with_source_change(self, db_with_andrew):
        _insert_wearable(db_with_andrew, "andrew-001", _5D_AGO, "oura", hrv=65)
        _insert_wearable(db_with_andrew, "andrew-001", _1D_AGO, "garmin", hrv=50)
...
class TestValidationInScheduler:
    ...
    def test_dry_run_message_annotated_on_source_change(self, mock_engagement, mock_audit, mock_persons, mock_now, mock_context, mock_compose, db_with_andrew):
        ...
        mock_now.return_value = datetime.now().replace(hour=7, minute=10, tzinfo=ZoneInfo("America/Los_Angeles"))
