...
mcp_server/
  ...
  tools.py         59+ tool implementations + registry
...
- Run tests: `python3 -m pytest tests/ -v` — all 880+ must pass
...
- Hardcoded absolute dates in test fixtures rot. If the code-under-test queries `datetime('now', '-N hours')`, the fixture must compute its timestamps relative to `datetime.now()`, not pin a literal date. `test_voice_bridge.py::test_filters_raw_voice_keeps_summary` sat red because its fixture aged out of the 168h window. ~16 other test files in this repo still hardcode 2026 dates — audit when convenient. (learned 2026-04-12)
