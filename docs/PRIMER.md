...
416 tests passing. Sync protocol tested: push/pull, conflict resolution, soft deletes, multi-entity.

## What's Coming

In order of priority:
1. Per-user API tokens (currently one shared token, needs isolation)
2. JWT auth (HMAC-SHA256, 1hr access + 7d refresh)
3. Litestream backups (SQLite WAL to Cloudflare R2)
4. v1 audit logging
5. Supabase migration at 5-10 users (swap db.py internals, API stays the same)
...
| `tests/test_v1_api.py` | 17 sync + CRUD tests |
...
python3 -m pytest tests/test_v1_api.py -v    # 17 tests
